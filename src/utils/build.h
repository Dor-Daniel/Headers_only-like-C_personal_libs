...
#define __maybe_unused __attribute__((unused))
    ...
    #define COMPILER_PATH               L"\"C:\\msys64\\new_install\\mingw64\\bin\\gcc.exe\" "
#endif

#ifndef COMPILER_PATH_EXT
    #define COMPILER_PATH_EXT               "\"C:\\\\msys64\\\\new_install\\\\mingw64\\\\bin\\\\gcc.exe\" "
#endif
#ifndef DEBUGGER_PATH_EXT
    #define DEBUGGER_PATH_EXT              "\"C:\\\\msys64\\\\new_install\\\\usr\\\\bin\\\\gdb.exe\""
#endif
#ifndef LINKER_FLAGS
    #define LINKER_FLAGS                L"-lglfw3dll -lopengl32 -lgdi32 -lshlwapi"
#endif
#ifndef COMPILER_FLAGS 
    #define COMPILER_FLAGS              L"-Wall -Wextra -Werror -Wno-int-to-pointer-cast -Wno-unused-function"
#endif
#ifndef LIBS_FLAGS_PATH 
    #define LIBS_FLAGS_PATH             L"-L\"C:\\Libraries\\glfw-3.4.bin.WIN64 (1)\\glfw-3.4.bin.WIN64\\lib-mingw-w64\" "
#endif
#ifndef LIBS_INCLUDE_PATH 
    #define LIBS_INCLUDE_PATH           L"-I\"C:\\Libraries\\glfw-3.4.bin.WIN64 (1)\\glfw-3.4.bin.WIN64\\include\" "
#endif
...
#endif
#ifndef VSCODE_DIR_PATH
    #define VSCODE_DIR_PATH ".vscode"
#endif
...
#ifndef SOURCE_FILES_DIRECTORY_REG
    #define SOURCE_FILES_DIRECTORY_REG                 "src"
#endif
#define SOURCE_FILES_DIRECTORY_ALL             SOURCE_FILES_DIRECTORY L"\\*"
#ifndef BUILD_DIRECTORY
    #define BUILD_DIRECTORY                        L"build"
#endif
#ifndef BUILD_DIRECTORY_REG
    #define BUILD_DIRECTORY_REG                      "build"
#endif
...
void builder___generate_template();
...
void builder___generate_template()
{
    if(CreateDirectoryA(VSCODE_DIR_PATH, NULL) == 0)
    {
        DWORD err = GetLastError();
        printf(
            "Could not create `%s` directory. because: %s\naborting...\n", 
            VSCODE_DIR_PATH,
            err == ERROR_ALREADY_EXISTS ? "ALREADY_EXISTS" : "UNKNOWN" 
        );
        exit(1);
    }
    
    HANDLE tasks_handle = CreateFileA(
        VSCODE_DIR_PATH "/tasks.json", 
    ...
    if (tasks_handle == INVALID_HANDLE_VALUE)
    {
        DWORD err = GetLastError();
        printf("Could not create `%s` file. aborting...\n", VSCODE_DIR_PATH "/tasks.json");
        exit(1);
    }
    
    HANDLE launch_handle = CreateFileA(
        VSCODE_DIR_PATH "/launch.json", 
        GENERIC_READ | GENERIC_WRITE, 
        FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE, 
        NULL,
        CREATE_NEW,
    ...
    if (launch_handle == INVALID_HANDLE_VALUE)
    {
        printf("Could not create `%s` file. aborting...\n", VSCODE_DIR_PATH "/launch.json");
        exit(1);
    }
    
    HANDLE c_cpp_props_handle = CreateFileA(
        VSCODE_DIR_PATH "/c_cpp_properties.json", 
        GENERIC_READ | GENERIC_WRITE, 
        FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE, 
        NULL,
        CREATE_NEW,
    ...
    if (c_cpp_props_handle == INVALID_HANDLE_VALUE)
    {
        printf("Could not create `%s` file. aborting...\n", VSCODE_DIR_PATH "/c_cpp_properties.json");
        exit(1);
    }

    HANDLE settings_handle = CreateFileA(
    ...
    if(CreateDirectoryW(SOURCE_FILES_DIRECTORY, NULL) == 0)
    {
        printf("Could not create `%ls` directory. aborting...\n", SOURCE_FILES_DIRECTORY);
        exit(1);
    }
    
    HANDLE main_handle = CreateFileW(
        SOURCE_FILES_DIRECTORY L"/main.c", 
        GENERIC_READ | GENERIC_WRITE, 
        FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE, 
        NULL,
        CREATE_NEW,
    ...
    if (main_handle == INVALID_HANDLE_VALUE)
    {
        printf("Could not create `%ls` file. aborting...\n", SOURCE_FILES_DIRECTORY L"/main.c");
        exit(1);
    }
    ...
    "    return 0;\n"
    "}\n";
    unsigned long wrriten_bytes_count;
    WriteFile(main_handle, main_content, strlen(main_content), &wrriten_bytes_count, NULL);
    
    if(CreateDirectoryW(BUILD_DIRECTORY, NULL) == 0)
    {
        printf("Could not create `%ls` directory. aborting...\n", BUILD_DIRECTORY);
        exit(1);
    }

    if(CreateDirectoryW(BUILD_DIRECTORY L"/debug", NULL) == 0)
    {
        printf("Could not create `%ls` directory. aborting...\n", BUILD_DIRECTORY "/debug");
        exit(1);
    }

    if(CreateDirectoryW(BUILD_DIRECTORY L"/release", NULL) == 0)
    {
        printf("Could not create `%ls` directory. aborting...\n", BUILD_DIRECTORY "/release");
        exit(1);
    }
    
    char c_cpp_props_file_prefix [] = 
    "{\n"
    "   \"configurations\": [\n"
    "       {\n"
    ...
    "           \"includePath\": [\n"
    "               \"${workspaceFolder}\",\n"
    "               \"${workspaceFolder}/" SOURCE_FILES_DIRECTORY_REG "/**\"\n"
    "           ],\n"
    "           \"defines\": [\n"
    "               \"_DEBUG\",\n"
    "               \"UNICODE\",\n"
    ...
    "           \"compilerPath\": " COMPILER_PATH_EXT ",\n"
    ...
    WriteFile(c_cpp_props_handle, c_cpp_props_file_prefix, strlen(c_cpp_props_file_prefix), &wrriten_bytes_count, NULL);
    ...
    "            \"name\": \"Debug & Clean\",\n"
    "            \"type\": \"cppdbg\",\n"
    "            \"program\": \"${workspaceFolder}/" BUILD_DIRECTORY_REG "/debug/main.exe\",\n"
    "            \"args\": [],\n"
    "            \"stopAtEntry\": false,\n"
    "            \"cwd\": \"${workspaceFolder}\",\n"
    "            \"environment\": [],\n"
    ...
    "            \"MiMode\": \"gdb\",\n"
    "            \"miDebuggerPath\":" DEBUGGER_PATH_EXT ",\n"
    ...
    WriteFile(launch_handle, launch_prefix, strlen(launch_prefix), &wrriten_bytes_count, NULL);

    const char tasks_file_content [] = 
    "{\n"
    "    \"tasks\" : \n"
    "    [\n"
    "        {\n"
    ...
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"args\": [ \"+br\" \"+r\" ],\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Build\",\n"
    "        },\n"
    ...
    "            \"group\": \"Build\",\n"
    "            \"presentation\": {\n"
    "                \"reveal\": \"never\",    // Prevents the terminal panel from popping open\n"
    "                \"focus\": false,       // Keeps your cursor in the editor window\n"
    "                \"panel\": \"shared\"     // Reuses the same background panel\n"
    "            }\n"
    "        },\n"
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Informational-Build (Release)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"args\": [\"+br\", \"+i\"],\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Build\",\n"
    ...
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Informational-Build (Debug)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"args\": [\"+bd\", \"+i\"],\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Build\",\n"
    ...
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Run (Release)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\" BUILD_DIRECTORY_REG "\\\\release\\\\main.exe\",\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Run\",\n"
    "        },\n"
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Run (Debug)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\" BUILD_DIRECTORY_REG "\\\\debug\\\\main.exe\",\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Run\",\n"
    "        },\n"
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Clean-Cache & Rebuild (Release).\",\n"
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"args\": [ \"+br\", \"+c\", \"+i\" ],\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Rebuild\",\n"
    ...
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Clean-Cache & Rebuild (Debug)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"args\": [ \"+bd\", \"+c\", \"+i\" ],\n"
    "            \"group\": \"Rebuild\",\n"
    ...
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Clean-Cache & Rebuild & Run (Release)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"args\": [\"+br\", \"+r\", \"+c\"],\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Rebuild\",\n"
    ...
    "        {\n"
    "            \"type\": \"shell\",\n"
    "            \"label\": \"Clean-Cache & Rebuild & Run (Debug)\",\n"
    "            \"command\": \"${workspaceFolder}\\\\builder.exe\",\n"
    "            \"args\": [\"+bd\", \"+r\", \"+c\"],\n"
    "            \"problemMatcher\": [\"$gcc\"],\n"
    "            \"group\": \"Rebuild\",\n"
    ...
    WriteFile(tasks_handle, tasks_file_content, strlen(tasks_file_content), &wrriten_bytes_count, NULL);
    
    const char settings_file_content [] =
    "{\n"
    "    \"code-runner.executorMap\": {\n"
    "        \"c\": \".\\\\"BUILD_DIRECTORY_REG "\\\\debug\\\\main.exe\"\n"
    "    },\n"
    ...
    WriteFile(settings_handle, settings_file_content, strlen(settings_file_content), &wrriten_bytes_count, NULL);
    
    CloseHandle(tasks_handle);
    CloseHandle(launch_handle);
    CloseHandle(c_cpp_props_handle);
